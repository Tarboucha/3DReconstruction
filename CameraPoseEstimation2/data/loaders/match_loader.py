    ...
    try:
        ...
        if reconstruct_kp and 'keypoints1' in result and 'keypoints2' in result:
            ...
            try:
                ...
                print(f"Warning: Could not reconstruct keypoints: {e}")
        ...
        print(f"⚠️  Error reconstructing match data: {e}")
        import traceback
        traceback.print_exc()
        return None
...
if __name__ == "__main__":
    print("="*70)
    print("MATCH LOADER MODULE - Testing")
    print("="*70)
    ...
    print("Standardized:   ", [f"{s:.3f}" for s in standardized])
    print("✓ Distance scores normalized (inverted)")
    ...
    print("Standardized:     ", [f"{s:.3f}" for s in standardized])
    print("✓ Confidence scores preserved")
    
    # Test pair quality computation
    print("\n" + "-"*70)
    pair_quality = standardizer.compute_pair_quality(standardized, len(standardized))
    print(f"Overall pair quality: {pair_quality:.3f}")
    print("✓ Pair quality computed")
    
    # Test 2: Match data reconstruction
    print("\n" + "="*70)
    print("Test 2: Match Data Reconstruction")
    print("-"*70)
    ...
    if match_data:
        print(f"✓ Reconstructed match data:")
        print(f"  Method: {match_data.method}")
        print(f"  Matches: {match_data.num_matches}")
        print(f"  Pair quality: {match_data.standardized_pair_quality:.3f}")
        print(f"  Match qualities: {[f'{m.standardized_quality:.3f}' for m in match_data.matches]}")
        
        # Test backward compatibility
        pts1 = match_data.pts1
        pts2 = match_data.pts2
        print(f"  Points shape: {pts1.shape} / {pts2.shape}")
        print("✓ Backward compatible access works")
    else:
