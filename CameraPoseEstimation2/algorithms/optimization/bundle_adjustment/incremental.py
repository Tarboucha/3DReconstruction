...
class IncrementalBundleAdjustment(BaseOptimizer):
    ...
    def optimize(self,
        ...
        print(f"Incremental BA after adding {new_camera_id}:")
        print(f"  Optimizing {len(camera_ids_to_optimize)}/{len(camera_ids)} cameras")
        print(f"  Optimizing {points_3d.shape[1]} points")
...
def adjust_after_new_camera(reconstruction_state: Dict,
    ...
    if result.success:
        ...
        print(f"Warning: Incremental BA failed: {result.metadata.get('error')}")
        return reconstruction_state