...
class ProgressiveIntrinsicsLearner(BaseOptimizer):
    ...
    def initialize_intrinsics(self,
        ...
        print(f"Initialized intrinsics for {camera_id}:")
        print(f"  {estimate}")
    ...
    def refine_intrinsics(self,
        ...
        if len(points_3d) < self.config.MIN_OBSERVATIONS:
            print(f"  {camera_id}: Insufficient observations ({len(points_3d)}), keeping current")
            return self.current_intrinsics.get(camera_id)
        
        # Get current estimate
        current = self.current_intrinsics.get(camera_id)
        if current is None:
            print(f"  {camera_id}: No current intrinsics, cannot refine")
            return None
        
        print(f"\nRefining intrinsics for {camera_id}:")
        print(f"  Current: {current}")
        print(f"  Observations: {len(points_3d)}")
        ...
        try:
            ...
            print(f"  Blended: {blended_estimate}")
    ...
    def update_camera_intrinsics(self,
        ...
        if camera_id not in self.current_intrinsics:
            print(f"No intrinsics learned for {camera_id}")
        ...
        if camera_id in reconstruction_state['cameras']:
            ...
            print(f"Updated intrinsics for {camera_id} in reconstruction")
