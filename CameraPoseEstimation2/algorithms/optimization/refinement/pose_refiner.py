...
class PoseRefiner(BaseOptimizer):
    ...
    def refine_pose(self,
        ...
        print(f"Refining pose for {camera_id}:")
        ...
        print(f"  Initial cost: {initial_cost:.3f}px")
        ...
        try:
            ...
            print(f"  Final cost: {final_cost:.3f}px ({result.nfev} iterations, {runtime:.2f}s)")
            print(f"  Cost reduction: {initial_cost - final_cost:.3f}px")
    ...
    def refine_multiple_poses(self,
        ...
        for cam_id, cam_data in cameras.items():
            if cam_id not in observations or len(observations[cam_id]) == 0:
                print(f"  {cam_id}: No observations, skipping")
                continue
            ...
            if len(cam_points_3d) < 4:
                print(f"  {cam_id}: Insufficient points ({len(cam_points_3d)}), skipping")
                continue
