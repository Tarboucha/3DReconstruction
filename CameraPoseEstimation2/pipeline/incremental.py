...
from algorithms.geometry.triangulation import TriangulationEngine
from algorithms.selection.pair_selection import InitializationPairSelector
...
from .essential_estimation import MatrixEstimationConfig
...
class MainPosePipeline:
    ...
    def __init__(self, provider: Optional[IMatchDataProvider] = None):
        """
        Initialize reconstruction pipeline.
        
        Args:
            provider: Data provider for accessing match data.
                    Can be set later with set_provider().
        """
        # Store provider
        ...
        self.incremental_bundle_adjuster = IncrementalBundleAdjuster(ba_config)
        self.global_bundle_adjuster = GlobalBundleAdjuster(ba_config)
        
        # State variables
        self.reconstruction = None
        self.registered_images = set()
        
        # Inject provider into components if provided
    ...
    def _inject_provider(self):
        """Inject provider into all algorithm components"""
        if self.provider is None:
            return
        
        self.triangulation_engine.set_provider(self.provider)
        self.pair_selector.set_provider(self.provider)

    def process_monument_reconstruction(self, 
                                   provider: IMatchDataProvider = None,
                                   output_directory='./pose_results',
                                   chosen_images=None):
        ...
        if self.provider is None:
            raise ValueError(
                ...
                "Pass provider to constructor or this method.\n"
                "Example: pipeline.process_monument_reconstruction(provider=my_provider)"
            )
        
        # Validate provider
        print("Validating data provider...")
        validation = self.provider.validate()
        if validation:
            print(" Provider validation passed")
        else:
            print("  Provider validation warnings:")
            validation.print_report()
        
        print(f" Dataset: {self.provider.get_image_count()} images, "
            f"{self.provider.get_pair_count()} pairs")
    ...
    def _initialize_two_view_with_ba(self, chosen_images=None):
        ...
        print("\n" + "="*80)
        print("INITIALIZATION: Two-View Reconstruction with Provider")
        print("="*80)
        
        # Select best pair using provider
        if chosen_images is None:
            # Automatic selection using pair selector
            print("Selecting best initialization pair...")
            best_pair_key = self.pair_selector.get_best_pair_for_pipeline()
            
            # Get match data for selected pair
            match_data = self.provider.get_match_data(best_pair_key)
            
            image1, image2 = best_pair_key
            pts1 = match_data.pts1
            pts2 = match_data.pts2
            
            print(f"✓ Selected: {image1} <-> {image2}")
            print(f"  Matches: {match_data.num_matches}")
            print(f"  Quality: {match_data.standardized_pair_quality:.3f}")
            
        else:
            # Manual pair selection
            image1, image2 = chosen_images
            pair_key = (image1, image2)
            
            print(f"Using specified pair: {image1} <-> {image2}")
            
            # Check if pair exists (try both orientations)
            if self.provider.has_pair(pair_key):
                match_data = self.provider.get_match_data(pair_key)
                pts1 = match_data.pts1
                pts2 = match_data.pts2
            elif self.provider.has_pair((image2, image1)):
                match_data = self.provider.get_match_data((image2, image1))
                # Swap points for reversed pair
                pts1 = match_data.pts2
                pts2 = match_data.pts1
            else:
                raise ValueError(
                    f"Specified pair not found: {image1} <-> {image2}\n"
                    f"Available images: {list(self.provider.get_all_images())[:10]}..."
                )
            
            print(f"✓ Matches: {match_data.num_matches}")
            print(f"  Quality: {match_data.standardized_pair_quality:.3f}")
        
        # 2. Get image information from provider
        img1_info = self.provider.get_image_info(image1)
        img2_info = self.provider.get_image_info(image2)
        
        img1_size = img1_info['size']
        img2_size = img2_info['size']
        ...
        essential_result = self.essential_estimator.estimate(
            pts1, pts2, 
            image_size1=img1_size,
            image_size2=img2_size
        )
        ...
        if not essential_result['success']:
            ...
            K1_init = self.essential_estimator.estimate_camera_matrix(img1_size)
            K2_init = self.essential_estimator.estimate_camera_matrix(img2_size)
        else:
            ...
            print(f"✓ Initial estimation successful:")
        ...
        if essential_result['success']:
            ...
            inlier_pts1 = pts1[mask]  # 🔧 FIXED: Use pts1, not best_pair['pts1']
            inlier_pts2 = pts2[mask]  # 🔧 FIXED: Use pts2, not best_pair['pts2']
        else:
            inlier_pts1 = pts1  # 🔧 FIXED
            inlier_pts2 = pts2  # 🔧 FIXED
        
        triangulated_mask = np.zeros(len(inlier_pts1), dtype=bool)
        ...
        refinement_result = self.iterative_adjuster.iterative_refinement_with_relaxation(
            ...
            K1_init=K1_init,
            K2_init=K2_init,
            image_size1=img1_size,  # 🔧 FIXED: Use img1_size directly
            image_size2=img2_size   # 🔧 FIXED: Use img2_size directly
        )
        
        # 5. Use refined results or fall back to initial
        if refinement_result and refinement_result.get('success'):
            ...
            triangulated_mask = refinement_result.get('triangulated_mask', triangulated_mask)
        ...
        # 6. Add cameras to reconstruction with final parameters
        ...
        # 7. Handle triangulation (if not already done by refinement)
        if not skip_triangulation:
            ...
            triangulated_result = self.triangulation_engine.triangulate_initial_points(
                ...
                R_final,
                t_final,
                [K1_final, K2_final],  # 🔧 FIXED: Pass as list
                (image1, image2)  # 🔧 FIXED: Use tuple of image IDs
            )
            ...
            print(f"  Success rate: {100.0 * initial_point_count / len(inlier_pts1):.1f}%")
        
        # 8. Add points and observations to reconstruction
        print(f"\n=== ADDING {points_3d.shape[1]} POINTS TO RECONSTRUCTION ===")
        ...
        if num_rejected > 0:
            ...
            # Calculate adaptive depth bounds
            ...
            retriangulated_result = retriangulation_engine.triangulate_initial_points(
                ...
                R2=cam2['R'],
                t2=cam2['t'],
                K=[cam1.get('K', K1_final), cam2.get('K', K2_final)],  # 🔧 FIXED
            ...
            print(f"✓ Recovered: {num_recovered}/{num_rejected} points")
            ...
            if num_recovered > 0:
                ...
                # Add observations using retriangulated_result
        ...
        # 14. Update reconstruction from validated state
        ...
        bootstrap_points_added = 0  # Initialize
        
        print(f"Bootstrapping with {len(bootstrap_images)} additional images...")
        ...
        print(f"  Recovered after BA: {num_recovered}")
        print(f"  Bootstrap triangulation: {bootstrap_points_added}")
    ...
    def _add_views_with_incremental_ba(self):
        ...
        processed_images = set(self.reconstruction.cameras.keys())
        remaining_images = set(self.provider.get_image_info().keys()) - processed_images
    ...
    def _get_matches_with_existing_images(self, new_image: str, 
                                        existing_images: Set[str]) -> List[Dict]:
        """
        Find all matches between new image and existing images in reconstruction.
        
        Args:
            new_image: ID of the new image to add
            existing_images: Set of already registered image IDs
        
        Returns:
            List of match information dictionaries
        """
        matches_with_existing = []
            ...
            for pair_key in pair_keys:
                if self.provider.has_pair(pair_key):
                    ...
                    if match_data.num_matches >= 8:
                        matches_with_existing.append({
                            'pair_key': pair_key,
                            'existing_image': existing_image,
                            'new_image': new_image,
                            'match_data': match_data,
                            'candidate_is_first': pair_key[0] == new_image
                        })
                    break  # Found match in one orientation, don't check the other
        
        return matches_with_existing
    ...
    def _estimate_rough_pose_for_bootstrap(
        ...
        try:
            ...
            anchor_image_size = self.provider.get_image_info()[anchor_camera]['size']
            bootstrap_image_size = self.provider.get_image_info()[bootstrap_image]['size']
    ...
    def _convert_to_json_serializable(self, data: Dict) -> Dict:
        ...
        Args:
            data: Dictionary containing reconstruction data
    ...
    def _export_colmap_format(self, export_data: Dict, output_directory: str):
        """
        Export reconstruction in COLMAP text format.
        ...
        Args:
            export_data: Reconstruction data dictionary
            output_directory: Output directory path
        """
        colmap_dir = os.path.join(output_directory, 'colmap')
        ...
        with open(cameras_file, 'w') as f:
            ...
            camera_id = 1
            for img_id, pose_data in export_data['camera_poses'].items():
                K = pose_data['K']
                width = pose_data['image_size'][0] if pose_data['image_size'] else 1920
                height = pose_data['image_size'][1] if pose_data['image_size'] else 1080
        ...
        with open(images_file, 'w') as f:
            ...
            for img_id, pose_data in export_data['camera_poses'].items():
                ...
                camera_id = image_id  # One camera per image for now
                
                f.write(f"{image_id} {qw} {qx} {qy} {qz} {tx} {ty} {tz} {camera_id} {img_id}\n")
                f.write("\n")  # Empty line for points2D (not implemented here)
        ...
        with open(points3d_file, 'w') as f:
            ...
            points_3d = export_data['points_3d']
            for i in range(points_3d.shape[1]):
                x, y, z = points_3d[:, i]
                # Default color (gray) and error
                r, g, b = 128, 128, 128
                error = 0.0
                
                f.write(f"{i} {x} {y} {z} {r} {g} {b} {error}\n")
        
        print(f"COLMAP format exported to: {colmap_dir}")
    ...
    def _create_summary_report(self, export_data: Dict, report_file: str):
        """
        Create a detailed text summary of the reconstruction.
        ...
        with open(report_file, 'w') as f:
            f.write("="*70 + "\n")
            f.write("RECONSTRUCTION SUMMARY REPORT\n")
            f.write("="*70 + "\n\n")
            
            # Metadata
            metadata = export_data.get('reconstruction_metadata', {})
            if 'processing_timestamp' in metadata:
                f.write(f"Timestamp: {metadata['processing_timestamp']}\n\n")
            
            # Statistics
            stats = metadata.get('statistics', {})
            f.write("RECONSTRUCTION STATISTICS\n")
            f.write("-" * 70 + "\n")
            f.write(f"Number of cameras: {stats.get('num_cameras', 0)}\n")
            f.write(f"Number of 3D points: {stats.get('num_points', 0)}\n")
            f.write(f"Number of observations: {stats.get('num_observations', 0)}\n")
            f.write(f"Mean track length: {stats.get('mean_track_length', 0):.2f}\n")
            f.write(f"Max track length: {stats.get('max_track_length', 0)}\n")
            f.write(f"Mean reprojection error: {stats.get('mean_reprojection_error', 0):.3f} px\n")
            f.write(f"Median reprojection error: {stats.get('median_reprojection_error', 0):.3f} px\n\n")
            
            # Camera details
            f.write("CAMERA DETAILS\n")
            f.write("-" * 70 + "\n")
            
            for img_id, pose_data in sorted(export_data['camera_poses'].items()):
                f.write(f"\nCamera: {img_id}\n")
                
                K = pose_data['K']
                f.write(f"  Focal length: fx={K[0,0]:.2f}, fy={K[1,1]:.2f}\n")
                f.write(f"  Principal point: cx={K[0,2]:.2f}, cy={K[1,2]:.2f}\n")
                
                world_pos = pose_data['world_position']
                f.write(f"  World position: [{world_pos[0]:.3f}, {world_pos[1]:.3f}, {world_pos[2]:.3f}]\n")
                
                if pose_data['image_size']:
                    f.write(f"  Image size: {pose_data['image_size'][0]} x {pose_data['image_size'][1]}\n")
            ...
            if points_3d.shape[1] > 0:
                ...
                min_coords = np.min(points_3d, axis=1)
                max_coords = np.max(points_3d, axis=1)
                center = np.mean(points_3d, axis=1)
                ...
                f.write(f"Centroid: [{center[0]:.3f}, {center[1]:.3f}, {center[2]:.3f}]\n")
                
                # Calculate spread
                distances = np.linalg.norm(points_3d - center.reshape(-1, 1), axis=0)
                f.write(f"\nPoint spread from centroid:\n")
                f.write(f"  Mean: {np.mean(distances):.3f}\n")
                f.write(f"  Std: {np.std(distances):.3f}\n")
                f.write(f"  Max: {np.max(distances):.3f}\n")
