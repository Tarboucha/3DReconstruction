...
class OptimizationResult:
    ...
    def print_summary(self):
        ...
        print("="*60)
        print(f"Status: {'✅ ' if self.success else '❌ '}{self.status.value}")
        print(f"Iterations: {self.num_iterations}")
        print(f"Runtime: {self.runtime:.3f}s")
        
        print(f"\nCost:")
        print(f"  Initial: {self.initial_cost:.6f}")
        print(f"  Final: {self.final_cost:.6f}")
        print(f"  Reduction: {self.get_cost_reduction():.6f} "
              f"({self.get_relative_cost_reduction():.2%})")
        
        if self.residuals is not None:
            print(f"\nResiduals:")
            print(f"  Mean: {np.mean(self.residuals):.6f}")
            print(f"  Median: {np.median(self.residuals):.6f}")
            print(f"  Max: {np.max(self.residuals):.6f}")
