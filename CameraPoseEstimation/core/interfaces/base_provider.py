...
class IMatchDataProvider(ABC):
    ...
    def print_summary(self):
        ...
        print("="*60)
        logger.info("MATCH DATA PROVIDER SUMMARY")
        print("="*60)
        logger.info(f"Images: {stats.get('num_images', 'N/A')}")
        logger.info(f"Pairs: {stats.get('num_pairs', 'N/A')}")
        logger.info(f"Avg matches/pair: {stats.get('avg_matches_per_pair', 'N/A'):.1f}")
