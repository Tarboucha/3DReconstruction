...
class StructuredMatchDataProvider(IMatchDataProvider):
    ...
    def __init__(self, 
        ...
        logger.info(f"✓ StructuredDataProvider initialized")
        logger.info(f"  Directory: {results_dir}")
        logger.info(f"  Pairs indexed: {len(self._pair_to_file)}")
        logger.info(f"  Images: {len(self._image_info)}")
        logger.info(f"  Cache size: {cache_size}")
        if min_quality:
            logger.info(f"  Quality filter: ≥{min_quality}")
        if min_matches:
            logger.info(f"  Match count filter: ≥{min_matches}")
    


    def _build_index(self):
        """Build index of pairs to files without loading all data"""
        logger.info(f"Building index from: {self.results_dir}")
        ...
        for batch_file in batch_files:
            try:
                ...
                logger.info(f"  Warning: Could not index {batch_file.name}: {e}")
                continue
        
        logger.info(f"  Indexed {len(self._pair_to_file)} valid pairs")
    ...
    def _preload_all(self):
        ...
        logger.info("Preloading all pairs...")
        ...
        logger.info(f"✓ Preloaded {len(self._pair_to_file)} pairs")
