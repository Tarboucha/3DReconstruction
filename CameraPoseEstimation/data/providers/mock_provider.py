...
class MockProvider(IMatchDataProvider):
    ...
    def __init__(self,
        ...
        logger.info(f"✓ MockProvider initialized")
        logger.info(f"  Images: {len(self._image_names)}")
        logger.info(f"  Pairs: {len(self._pairs)}")
        logger.info(f"  Avg matches/pair: {self.matches_per_pair}")
...
if __name__ == "__main__":
    ...
    for pair in best:
        ...
        logger.info(f"  {pair}: quality={data.standardized_pair_quality:.3f}, matches={data.num_matches}")
    ...
    for pair in best[:2]:
        ...
        logger.info(f"\nPair: {pair}")
        logger.info(f"  Matches: {len(match_data.matches)}")
