...
class MainPosePipeline:
    ...
    def _initialize_two_view_with_ba(self, chosen_images=None):
        ...
        essential_result = self.essential_estimator.estimate(
            ...
            image_size1=self.matches_pickle['image_info'][best_pair['image_pair'][0]]['size'], 
            image_size2=self.matches_pickle['image_info'][best_pair['image_pair'][1]]['size']
        )
        
        if not essential_result['success']:
            raise RuntimeError(f"Essential matrix estimation failed using {essential_result['method']}")
        
        # Log quality information
        print(f"Essential matrix estimation:")
        ...
        print(f"  Quality: {essential_result['quality_assessment']}")
        ...
        # 4. Add cameras to reconstruction
        # First camera at origin
        self.reconstruction.add_camera(
            camera_id=image1,
            R=np.eye(3),
            t=np.zeros((3,1)),
            K=essential_result['camera_matrices'][0]
        )
        ...
        self.reconstruction.add_camera(
            ...
            R=pose_result['R'],
            t=pose_result['t'],
            K=essential_result['camera_matrices'][1]
        )
        
        # 5. Store ALL original correspondences for potential re-triangulation
        all_original_correspondences = {
            'pts1': inlier_pts1.copy(),
            'pts2': inlier_pts2.copy(),
            'image1': image1,
            'image2': image2
        }
        
        # 6. Initial triangulation with conservative thresholds
        ...
        # 7. Track which points were successfully triangulated (for re-triangulation later)
        # We need to determine which of the original correspondences made it through
        triangulated_mask = np.zeros(len(inlier_pts1), dtype=bool)
        if 'observations' in triangulated_result:
            # Use observations to determine which points were kept
            obs_indices = set()
            for obs in triangulated_result['observations']:
                if 'original_index' in obs:
                    obs_indices.add(obs['original_index'])
            for idx in obs_indices:
                if idx < len(triangulated_mask):
                    triangulated_mask[idx] = True
        else:
            # Fallback: assume first N points were kept
            triangulated_mask[:initial_point_count] = True
        
        # 8. Add initial points and observations to reconstruction
        for i in range(points_3d.shape[1]):
            point_id = self.reconstruction.add_point(points_3d[:, i])
            
            # Find corresponding 2D observations
            # This assumes points are in order (may need adjustment based on your engine)
            if i < len(inlier_pts1):
                self.reconstruction.add_observation(image1, point_id, inlier_pts1[i])
                self.reconstruction.add_observation(image2, point_id, inlier_pts2[i])
        
        # Store initialization info
        self.reconstruction.initialization_info = {
            'method': essential_result['method'],
            'num_inliers': essential_result['num_inliers'],
            'inlier_ratio': essential_result['inlier_ratio'],
            'quality_assessment': essential_result['quality_assessment'],
            'camera_estimated': essential_result['camera_estimated'],
            'initial_triangulated': initial_point_count,
            'initial_rejected': len(inlier_pts1) - initial_point_count
        }
        
        # 9. Convert to legacy format for bundle adjustment
    ...
    def _select_best_next_image(self, remaining_images: Set[str]) -> str:
        ...
        for candidate in remaining_images:
            score_info = self._score_candidate_image(candidate)  # No reconstruction_state
            candidate_scores.append(score_info)
        
        # Remove candidates with zero score
        valid_candidates = [c for c in candidate_scores if c['total_score'] > 0]
        ...
        best_candidate = max(valid_candidates, key=lambda x: x['total_score'])
        selected_image = best_candidate['image']
        
        print(f"Selected: {selected_image}")
        print(f"  Score: {best_candidate['total_score']:.3f}")
        
        return selected_image
        
    def _score_candidate_image(self, candidate: str) -> Dict[str, Any]:
        """Score a candidate image using internal reconstruction"""
        existing_images = set(self.reconstruction.cameras.keys())
        
        # Find matches between candidate and existing images
        matches_with_existing = self._get_matches_with_existing_images(
            candidate, existing_images
        )
        ...
        if not matches_with_existing:
            return {
                ...
                'quality_score': 0.0,
                'geometry_score': 0.0,
                'potential_correspondences': 0
            }
        
        # Score 1: Correspondence potential
        correspondence_score = self._score_correspondence_potential(
            candidate, matches_with_existing
        )
        
        # Score 2: Match quality
        quality_score = self._score_match_quality(matches_with_existing)
        
        # Score 3: Geometric distribution
        geometry_score = self._score_geometric_distribution(matches_with_existing)
        
        # Combine scores
        total_score = (
            0.5 * correspondence_score +
            0.3 * quality_score +
            0.2 * geometry_score
        )
        ...
        return {
            ...
            'potential_correspondences': int(correspondence_score * 100)
        }
    
    def _get_matches_with_existing_images(self, candidate: str,
                                        existing_images: Set[str]) -> List[Dict]:
        """
        Get all feature matches between candidate image and existing images.
        """
        matches_with_existing = []
        
        for match_key, match_data in self.matches_pickle['matches_data'].items():
            # Parse match key to get image pair
            if isinstance(match_key, tuple):
                img1, img2 = match_key
            elif isinstance(match_key, str):
                if '_vs_' in match_key:
                    img1, img2 = match_key.split('_vs_')
                elif '_' in match_key and len(match_key.split('_')) == 2:
                    img1, img2 = match_key.split('_')
                else:
                    continue
            else:
                continue
            
            # Check if this match involves our candidate and an existing image
            if candidate == img1 and img2 in existing_images:
                matches_with_existing.append({
                    'existing_image': img2,
                    'candidate_image': candidate,
                    'match_data': match_data,
                    'candidate_is_first': True  # candidate is img1
                })
            elif candidate == img2 and img1 in existing_images:
                matches_with_existing.append({
                    'existing_image': img1,
                    'candidate_image': candidate,
                    'match_data': match_data,
                    'candidate_is_first': False  # candidate is img2
                })
        
        return matches_with_existing

    def _score_correspondence_potential(self, candidate: str,
                                   matches_with_existing: List[Dict]) -> float:
        """Score based on potential 2D-3D correspondences using internal reconstruction"""
        total_potential = 0
        ...
        for match_info in matches_with_existing:
            ...
            # Get observations from reconstruction
            ...
            if 'correspondences' in match_data:
                num_matches = len(match_data['correspondences'])
            elif match_data.get('keypoints1') is not None:
                num_matches = len(match_data['keypoints1'])
            elif 'pts1' in match_data:
                num_matches = len(match_data['pts1'])
            else:
                num_matches = 0
            
            num_observations = len(existing_observations)
            
            # Estimate potential correspondences
            potential = min(num_matches, num_observations) * 0.3
            total_potential += potential
        
        # Normalize to 0-1 score
        score = min(1.0, total_potential / 50.0)
        return score
    
    @staticmethod
    def _score_match_quality(matches_with_existing: List[Dict]) -> float:
        """
        Score based on the quality of feature matches.
        """
        if not matches_with_existing:
            return 0.0
        ...
        for match_info in matches_with_existing:
            ...
            # Extract quality metrics from match data
            if 'confidence_scores' in match_data:
                # LightGlue case - use confidence_scores
                quality_scores.extend(match_data['confidence_scores'])
            elif 'confidence' in match_data:
                # Alternative confidence field
                quality_scores.extend(match_data['confidence'])
            elif 'distances' in match_data:
                # Convert distances to quality (lower distance = higher quality)
                distances = np.array(match_data['distances'])
                max_dist = np.max(distances) if len(distances) > 0 else 1.0
                qualities = 1.0 - (distances / max_dist)
                quality_scores.extend(qualities)
            elif 'scores' in match_data:
                quality_scores.extend(match_data['scores'])
            else:
                # Default quality if no specific metric available
                # Get number of matches using updated logic
                if 'num_matches' in match_data:
                    num_matches = match_data['num_matches']
                elif 'correspondences' in match_data:
                    num_matches = len(match_data['correspondences'])
                elif match_data.get('keypoints1') is not None:
                    num_matches = len(match_data['keypoints1'])
                elif 'pts1' in match_data:
                ...
                quality_scores.extend([0.7] * num_matches)  # Default moderate quality
    ...
    @staticmethod
    def _score_geometric_distribution(matches_with_existing: List[Dict]) -> float:
        """
        Score based on how well distributed the matches are geometrically.
        """
        if not matches_with_existing:
            return 0.0
        ...
        for match_info in matches_with_existing:
            ...
            # Extract points for this specific match
            points = []
            
            if 'correspondences' in match_data:
                correspondences = match_data['correspondences']
                if match_info['candidate_is_first']:
                    points = [[corr[0], corr[1]] for corr in correspondences]
                else:
                    points = [[corr[2], corr[3]] for corr in correspondences]
            elif match_info['candidate_is_first']:
                if match_data.get('keypoints1') is not None:
                    points = [kp.pt for kp in match_data['keypoints1']]
                elif 'pts1' in match_data:
                    points = match_data['pts1']
            else:
                if match_data.get('keypoints2') is not None:
                    points = [kp.pt for kp in match_data['keypoints2']]
                elif 'pts2' in match_data:
                    points = match_data['pts2']
            
            if len(points) < 4:
                distribution_scores.append(0.3)  # Not enough points
                continue
            ...
            # Calculate distribution for this image pair
            std_x = np.std(points[:, 0])
            std_y = np.std(points[:, 1])
            
            # Get image size for normalization (if available)
            if 'image_size' in match_data:
                img_w, img_h = match_data['image_size']
                normalized_std = (std_x + std_y) / (img_w + img_h)
            else:
                # Fallback normalization
                normalized_std = (std_x + std_y) / 2000.0
            
            # Good distribution should have std around 0.1-0.3 of image size
            score = min(1.0, normalized_std / 0.2)
            distribution_scores.append(score)
        
        # Return average distribution score across all image pairs
