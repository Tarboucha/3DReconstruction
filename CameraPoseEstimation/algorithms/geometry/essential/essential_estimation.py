...
class MatrixEstimationConfig:
    ...
    RANSAC_THRESHOLD = 0.005        # Slightly higher for outdoor monument conditions
...
class EssentialMatrixEstimator(BaseEstimator):
    ...
    def _assess_essential_matrix_quality(self, E):
        """
        Assess quality with stricter, more realistic thresholds.
        """
        ...
        if s1 > 1e-6:
            ...
            # Two singular values should be very close (within 2%)
            sigma_diff = abs(sigma_ratio - 1.0)
            if sigma_diff < 0.01:  # Excellent
                quality_score += 0.5
            elif sigma_diff < 0.02:  # Good
                quality_score += 0.4
            elif sigma_diff < 0.05:  # Acceptable
                quality_score += 0.25
            else:
                warnings_list.append(f'Singular values differ significantly: σ2/σ1 = {sigma_ratio:.4f}')
            
            # Third singular value should be very small (< 1% for good, < 5% acceptable)
            if sigma3_ratio < 0.001:  # Excellent
                quality_score += 0.3
            elif sigma3_ratio < 0.01:  # Good
                quality_score += 0.2
            elif sigma3_ratio < 0.05:  # Acceptable
                quality_score += 0.1
            else:
                warnings_list.append(f'Third singular value too large: σ3/σ1 = {sigma3_ratio:.4f}')
        else:
        ...
        # More nuanced validity check
        is_valid = quality_score >= 0.7  # Require at least 70% quality
        ...
        return {
            ...
            'sigma_ratios': {
                's2/s1': float(sigma_ratio),
                's3/s1': float(sigma3_ratio)
            },
            'matrix_rank': rank,
            'warnings': warnings_list
    }
