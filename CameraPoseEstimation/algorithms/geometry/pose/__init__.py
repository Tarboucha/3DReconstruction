...
Examples:
    ...
    ...     logger.info(f"Translation: {result.data['t'].T}")
    ...     logger.info(f"Inliers: {result.data['inlier_count']}/{len(points_3d)}")
    ...     logger.info(f"Reprojection error: {result.data['reprojection_error']:.2f}px")
    ...
    ...     logger.info(f"Observations: {result['num_observations']}")
    ...     logger.info(f"Quality score: {result['validation']['quality_score']:.2f}")
...
logger = get_logger("geometry.pose")
    PoseValidator,
