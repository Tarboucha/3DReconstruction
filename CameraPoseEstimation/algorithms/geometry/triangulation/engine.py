...
class TriangulationEngine:
    ...
    def triangulate_initial_points(self, 
        ...
        # Guard against division by near-zero to prevent NaN/Inf
        points_3d = points_3d_hom[:3, :] / np.clip(np.abs(points_3d_hom[3, :]), 1e-10, None)  # 3xN
    ...
    def _triangulate_basic_new_points(self, 
        ...
        for existing_image in existing_cameras.keys():
            ...
            # Guard against division by near-zero to prevent NaN/Inf
            points_3d = points_3d_hom[:3, :] / np.clip(np.abs(points_3d_hom[3, :]), 1e-10, None)
    ...
    def _apply_hartley_sturm(self, pts1: np.ndarray, pts2: np.ndarray,
        ...
        try:
            ...
            # Compute essential matrix (for normalized coordinates!)
            E = self._compute_essential_matrix(R1, t1, R2, t2)
            
            # Apply Hartley-Sturm correction with essential matrix
            ...
            for i in range(len(pts1_norm)):
                pt1_corr, pt2_corr = self._correct_matching_points(
                    pts1_norm[i], pts2_norm[i], E  # ✅ Use E, not F!
                )
                pts1_corrected[i] = pt1_corr
                pts2_corrected[i] = pt2_corr
            
            # Convert back to pixel coordinates
            pts1_final = np.zeros((len(pts1_corrected), 2), dtype=np.float32)
            pts2_final = np.zeros((len(pts2_corrected), 2), dtype=np.float32)
            
            for i in range(len(pts1_corrected)):
                x1_n, y1_n = pts1_corrected[i]
                x2_n, y2_n = pts2_corrected[i]
                
                # Convert: pixel = K @ [normalized, 1]
                pts1_final[i, 0] = K1[0, 0] * x1_n + K1[0, 2]
                pts1_final[i, 1] = K1[1, 1] * y1_n + K1[1, 2]
                
                pts2_final[i, 0] = K2[0, 0] * x2_n + K2[0, 2]
                pts2_final[i, 1] = K2[1, 1] * y2_n + K2[1, 2]
            
            self.logger.debug(f"Hartley-Sturm corrected {len(pts1)} points")
            return pts1_final, pts2_final
            
        except Exception as e:
            self.logger.warning(f"Hartley-Sturm correction failed: {e}, using original points")
            return pts1.copy(), pts2.copy()
    ...
    def _correct_matching_points(self, pt1: np.ndarray, pt2: np.ndarray,
                                epipolar_matrix: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Correct matching points to satisfy epipolar constraint.
        Uses simplified Hartley-Sturm approach.
        
        Args:
            pt1: Point in first image (2D, pixel OR normalized)
            pt2: Point in second image (2D, pixel OR normalized)
            epipolar_matrix: F (for pixel coords) or E (for normalized coords)
        
        Returns:
            Corrected point pair (same coordinate system as input)
        """
        # Add homogeneous coordinate
        pt1_h = np.append(pt1, 1.0)
        pt2_h = np.append(pt2, 1.0)
        
        # Compute epipolar lines
        # l2 = epipolar_matrix * pt1 (line in image 2)
        # l1 = epipolar_matrix^T * pt2 (line in image 1)
        l2 = epipolar_matrix @ pt1_h
        l1 = epipolar_matrix.T @ pt2_h
        
        # Extract line coefficients ax + by + c = 0
        a1, b1, c1 = l1
        a2, b2, c2 = l2
        
        # Project points onto their epipolar lines
        # Closest point on line ax + by + c = 0 to point (x0, y0):
        # (x, y) = (x0, y0) - [(ax0 + by0 + c) / (a^2 + b^2)] * (a, b)
        
        # Correct point 1
        denom1 = a1**2 + b1**2
        if denom1 > 1e-10:
            correction1 = (a1 * pt1[0] + b1 * pt1[1] + c1) / denom1
            pt1_corr = pt1 - correction1 * np.array([a1, b1])
        else:
            pt1_corr = pt1.copy()
        ...
        denom2 = a2**2 + b2**2
        if denom2 > 1e-10:
            correction2 = (a2 * pt2[0] + b2 * pt2[1] + c2) / denom2
            pt2_corr = pt2 - correction2 * np.array([a2, b2])
        else:
            pt2_corr = pt2.copy()
