...
logger = get_logger("selection.initialization")
...
class InitializationPairSelector:
    ...
    def find_next_best_pairs(self, existing_cameras: Set[str], 
        ...
        logger.info(f"Finding next best pair(s) to add to {len(existing_cameras)} existing cameras...")
    ...
    def _find_best_pairs(self, matches_data: Optional[Dict] = None,
        ...
        logger.info(f"Evaluating {len(self.matches_data)} image pairs for {mode}...")
    ...
    def _print_top_candidates(self, num_candidates: int = 5, mode: str = "initialization"):
        ...
        for i, candidate in enumerate(self.pair_scores[:num_candidates]):
            ...
            logger.info(f"{i+1:2d}. {pair[0]} <-> {pair[1]}")
            logger.info(f"    Total Score: {score_result['total_score']:.3f}")
    ...
    def get_next_cameras_to_add(self, existing_cameras: Set[str],
        ...
        logger.info(f"Evaluating {len(candidate_cameras)} candidate cameras to add...")
        ...
        for i, candidate in enumerate(camera_scores[:min(5, len(camera_scores))]):
            logger.info(f"{i+1}. {candidate['camera_id']}")
    ...
    def score_image_pair(self, 
        ...
        try:
            ...
            logger.info(f"Warning: Unknown score type '{score_type_str}', defaulting to 'distance'")
