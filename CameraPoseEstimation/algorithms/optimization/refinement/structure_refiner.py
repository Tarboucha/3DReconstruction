...
logger = get_logger("optimization.structure_refiner")
...
# Add to CameraPoseEstimation2/algorithms/optimization/refinement/structure_refiner.py
    ...
    def refine_structure(self,
        ...
        logger.info(f"  Initial points: {initial_num_points}")
        ...
        if remove_outliers:
            ...
            logger.info(f"  After outlier removal: {points_3d.shape[1]} points ({outliers_removed} removed)")
        else:
            outliers_removed = 0
        ...
        logger.info(f"  After angle filtering: {points_3d.shape[1]} points ({angle_filtered} removed)")
        ...
        logger.info(f"  After track filtering: {points_3d.shape[1]} points ({track_filtered} removed)")
        ...
        if merge_points and self.config.ENABLE_MERGING:
            ...
            logger.info(f"  After merging: {points_3d.shape[1]} points ({merged_count} merged)")
        else:
            merged_count = 0
        ...
        logger.info(f"  Final points: {final_num_points}")
        ...
        logger.info(f"  Mean reprojection error: {metrics.mean_reprojection_error:.2f}px")
