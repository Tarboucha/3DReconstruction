...
logger = get_logger("optimization.progressive")
...
class ProgressiveRefinementPipeline(BaseOptimizer):
    ...
    def refine_reconstruction(self,
        ...
        logger.info("\n" + "="*70)
        logger.info("PROGRESSIVE REFINEMENT PIPELINE")
        logger.info("="*70)
        logger.info(f"Iterations: {num_iterations}")
        logger.info(f"Stages: {', '.join(self.config.REFINEMENT_ORDER)}")
        ...
        logger.info(f"  Points: {initial_num_points}")
        ...
        for iteration in range(num_iterations):
            ...
            logger.info(f"  Cost: {current_cost:.3f}px")
            logger.info(f"  Improvement: {improvement:.3f}px")
            logger.info(f"  Points: {points_3d.shape[1]}")
            logger.info(f"  Time: {iter_time:.2f}s")
        ...
        logger.info(f"  Points: {final_num_points} (removed {initial_num_points - final_num_points})")
        logger.info(f"  Final cost: {current_cost:.3f}px")
    ...
    def refine_after_camera_addition(self,
        ...
        if new_camera_id in cameras and new_camera_id in observations:
            logger.info(f"  Refining pose for {new_camera_id}...")
        ...
        if not light_refinement:
            logger.info("  Refining structure...")
