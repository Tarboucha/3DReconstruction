...
logger = get_logger("optimization.global_ba")
...
class GlobalBundleAdjustment(BaseOptimizer):
    ...
    def optimize(self,
        ...
        logger.info(f"\nInitial state:")
        logger.info(f"  Cost: {initial_cost:.3f}")
        logger.info(f"  Mean reprojection error: {initial_mean_error:.2f}px")
        ...
        try:
            ...
            logger.info(f"\nFinal state:")
            logger.info(f"  Cost: {final_cost:.3f}")
            logger.info(f"  Mean reprojection error: {final_mean_error:.2f}px")
            logger.info(f"\nImprovement:")
            logger.info(f"  Cost reduction: {initial_cost - final_cost:.3f} ({(initial_cost - final_cost)/initial_cost*100:.1f}%)")
            logger.info(f"  Error reduction: {initial_mean_error - final_mean_error:.2f}px")
            logger.info(f"{'='*70}\n")
            ...
            logger.error(f"Global BA failed: {str(e)}", exc_info=True)
...
def adjust_global(reconstruction_state: Dict,
    ...
    if result.success:
        ...
        logger.warning(f"Global BA failed: {result.metadata.get('error')}")
        return reconstruction_state