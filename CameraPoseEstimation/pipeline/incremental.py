...
# Geometry modules
from CameraPoseEstimation.algorithms.geometry.triangulation import TriangulationEngine
from CameraPoseEstimation.algorithms.geometry.essential import EssentialMatrixEstimator
from CameraPoseEstimation.algorithms.geometry.pose import PnPSolver, PoseEstimator

# Optimization modules
from CameraPoseEstimation.algorithms.optimization.bundle_adjustment import (
...
from CameraPoseEstimation.algorithms.optimization.refinement import (
    PoseRefiner,
...
# Selection modules
from CameraPoseEstimation.algorithms.selection.pair_selection import InitializationPairSelector
...
class Reconstruction:
    ...
    def save(self, filepath: str) -> None:
        ...
        logger.info(f"✓ Saved reconstruction to: {filepath}")
    ...
    def load(filepath: str) -> 'Reconstruction':
        ...
        with open(filepath, 'rb') as f:
            data = pickle.load(f)
        logger.info(f"✓ Loaded reconstruction from: {filepath}")
...
class IncrementalReconstructionPipeline:
    ...
    def run(self) -> Reconstruction:
        ...
        """
        logger.info("\n" + "=" * 70)
        logger.info(f"STARTING INCREMENTAL RECONSTRUCTION PIPELINE")
        logger.info("=" * 70)
        ...
        logger.info("=" * 70)
        logger.info(self.reconstruction.summary())
    ...
    def _initialize_two_view(self) -> None:
        ...
        K1_init, K2_init, initial_mask = self._estimate_initial_intrinsics(pts1, pts2, img_size1, img_size2)
        
        # 3. Refine with iterative optimization
        K1_final, K2_final, R_final, t_final, points_3d, valid_pts1, valid_pts2 = \
            self._refine_essential_matrix(pts1, pts2, K1_init, K2_init, img_size1, img_size2, initial_mask)
        ...
        logger.info(f"  Cameras: {len(self.reconstruction.cameras)}")
        logger.info(f"  Points: {len(self.reconstruction.points)}")
        logger.info(f"  Observations: {len(self.reconstruction.observations)}")
    ...
    def _select_and_prepare_initial_pair(self) -> Tuple:
        ...
        logger.info(f"✓ Selected pair: {img1} <-> {img2}")
        
        # Get score information
        score_result = pair_result['best_score_result']
        logger.info(f"  Score: {score_result.get('total_score', 0):.3f}")
        logger.info(f"  Matches: {score_result.get('num_matches', 0)}")
    ...
    def _estimate_initial_intrinsics(self,
        ...
        if not essential_result.get('success', False):
            logger.info(f"⚠ Initial estimation failed, using default intrinsics")
            ...
            logger.info(f"✓ Initial intrinsics estimated")
        
        return K1, K2, essential_result.get('inlier_mask', None)
    ...
    def _refine_essential_matrix(self,
                                 ...
                                 initial_inliers: Optional[np.ndarray] = None) -> Tuple:
        ...
        if refinement_result['success']:
            ...
            logger.info(f"⚠ Refinement failed, using fallback")
    ...
    def _fallback_estimation(self,
        ...
        logger.info(f"--- Fallback: Direct Estimation ---")
        ...
        try:
            ...
            if points_3d is None or points_3d.shape[1] == 0:
                raise RuntimeError("Triangulation produced no valid points")

            logger.info(f"✓ Fallback: {points_3d.shape[1]} points triangulated")

            return K1, K2, R, t, points_3d, valid_pts1, valid_pts2

        except Exception as e:
            logger.error(f"✗ Fallback triangulation failed: {e}")
            raise RuntimeError(f"Failed to triangulate initial points: {e}")
    ...
    def _add_initial_points(self,
        ...
        logger.info(f"--- Adding Points and Observations ---")
        ...
        logger.info(f"✓ Added {points_3d.shape[1]} points")
    ...
    def _run_two_view_bundle_adjustment(self) -> None:
        ...
        try:
            ...
            if ba_result.get('success', False):
                self._update_reconstruction_from_state(ba_result)
                logger.info(f"✓ Bundle adjustment completed")
                logger.info(f"  Final error: {ba_result.get('final_error', 'N/A'):.2f}px")
            else:
                logger.info(f"⚠ Bundle adjustment failed, using refined estimates")

        except Exception as e:
            logger.error(f"⚠ Bundle adjustment crashed: {type(e).__name__}: {e}")
            logger.info(f"  Continuing with current estimates")
    ...
    def _add_cameras_incrementally(self) -> None:
        ...
        logger.info(f"Registered: {len(registered_images)}")
        logger.info(f"Remaining: {len(unregistered_images)}")
        
        if not unregistered_images:
            logger.info(f"✓ All cameras already registered!")
            return
        ...
        while unregistered_images:
            iteration += 1
            logger.info(f"\n{'='*70}")
            logger.info(f"ITERATION {iteration}: Adding Camera {len(registered_images) + 1}/{len(all_images)}")
            logger.info(f"{'='*70}")
            ...
            if next_camera is None:
                logger.info(f"⚠ Cannot add more cameras (no valid candidates)")
                break
            ...
            if not success:
                logger.info(f"✗ Failed to add camera {next_camera}")
            ...
            logger.info(f"✓ Added {num_new_points} new 3D points")
            ...
            try:
                ...
                if ba_result.get('success', False):
                    self._update_reconstruction_from_state(ba_result)
                    logger.info(f"✓ Incremental BA completed")
                    logger.info(f"  Final error: {ba_result.get('final_error', 'N/A'):.2f}px")
                else:
                    logger.info(f"⚠ Incremental BA failed")

            except Exception as e:
                logger.error(f"⚠ Incremental BA crashed: {type(e).__name__}: {e}")
                logger.info(f"  Continuing without BA optimization")
            ...
            logger.info(f"  Total cameras: {len(self.reconstruction.cameras)}")
            logger.info(f"  Total points: {len(self.reconstruction.points)}")
            logger.info(f"  Total observations: {len(self.reconstruction.observations)}")
        ...
        logger.info(f"  Final: {len(self.reconstruction.cameras)} cameras, "
    ...
    def _select_next_camera(self, candidates: Set[str]) -> Optional[str]:
        ...
        if best_score >= 20:  # Minimum correspondences needed
            logger.info(f"  Selected: {best_camera} ({best_score} potential correspondences)")
            return best_camera
    ...
    def _add_camera_pnp(self, camera_id: str) -> bool:
        ...
        logger.info(f"--- Estimating Pose with PnP ---")
        ...
        if len(all_points_3d) < 10:
            logger.info(f"✗ Insufficient correspondences: {len(all_points_3d)}")
            return False
        
        points_3d = np.array(all_points_3d)
        points_2d = np.array(all_points_2d)
        
        logger.info(f"  Found {len(points_3d)} 3D-2D correspondences")
        ...
        try:
            ...
            if not pnp_result.get('success', False):
                logger.info(f"✗ PnP failed: {pnp_result.get('message', 'Unknown error')}")
                return False

        except Exception as e:
            logger.error(f"✗ PnP solver crashed: {type(e).__name__}: {e}")
            return False
        ...
        logger.info(f"✓ PnP solved successfully")
        logger.info(f"  Inliers: {np.sum(inliers)}/{len(points_3d)}")
        logger.info(f"  Reprojection error: {pnp_result.get('reprojection_error', 'N/A'):.2f}px")
        ...
        for existing_cam_id in self.reconstruction.cameras.keys():
            ...
            try:
                ...
                try:
                    ...
                    if points_3d is None or points_3d.shape[1] == 0:
                        logger.debug(f"  No valid points triangulated for {existing_cam_id}-{new_camera_id}")
                        continue

                except Exception as e:
                    logger.debug(f"  Triangulation failed for {existing_cam_id}-{new_camera_id}: {e}")
                    continue
    ...
    def _global_refinement(self) -> None:
        ...
        if refinement_result.success:
            logger.info(f"✓ Structure refined")
            logger.info(f"  Points before: {refinement_result.metadata.get('initial_points', 'N/A')}")
            logger.info(f"  Points after: {refinement_result.metadata.get('final_points', 'N/A')}")
            logger.info(f"  Removed: {refinement_result.metadata.get('points_removed', 0)}")
            ...
            logger.info(f"⚠ Structure refinement failed")
        ...
        try:
            ...
            if isinstance(ba_result, dict) and 'cameras' in ba_result and 'points_3d' in ba_result:
                ...
                logger.info(f"✓ Global bundle adjustment completed")
                hist = ba_result.get('optimization_history', [])
                if hist:
                    last = hist[-1]
                    logger.info(f"  Initial error: {last.get('initial_error', 'N/A')}")
                    logger.info(f"  Final error: {last.get('final_error', 'N/A')}")
                    logger.info(f"  Iterations: {last.get('iterations', 'N/A')}")
            else:
                logger.info(f"⚠ Global bundle adjustment did not return expected state")

        except Exception as e:
            logger.error(f"⚠ Global BA crashed: {type(e).__name__}: {e}")
            logger.info(f"  Continuing with current reconstruction state")
    ...
    def _assess_reconstruction_quality(self) -> None:
        ...
        logger.info(f"  Avg track length: {avg_track_length:.1f}")
        logger.info(f"  Avg obs/camera: {avg_obs_per_camera:.1f}")
        logger.info(f"  Avg reprojection error: {avg_reproj_error:.2f}px")
    ...
    def _export_results(self) -> None:
        ...
        logger.info(f"✓ Saved reconstruction: {reconstruction_file}")
    ...
    def _update_reconstruction_from_state(self, state: Dict) -> None:
        ...
        if 'points_3d' in state:
            ...
            if points_3d.shape[1] == len(self.reconstruction.points):
                ...
                logger.warning(f"Point count changed "
    ...
    def _export_colmap(self, output_dir: str) -> None:
        ...
        if not HAS_SCIPY:
            logger.info(f"  ⚠ Warning: scipy not available, using identity quaternions for rotation")
        ...
        with open(images_file, 'w') as f:
            ...
            for i, (cam_id, camera) in enumerate(self.reconstruction.cameras.items()):
                ...
                if HAS_SCIPY:
                    ...
                    logger.info(f"  Warning: Using identity quaternion for {cam_id} (scipy not available)")
...
if __name__ == "__main__":
    ...
    logger.info("="*70)
    logger.info(f"INCREMENTAL 3D RECONSTRUCTION PIPELINE")
    logger.info("="*70)
    logger.info(f"Match results: {MATCH_RESULTS_DIR}")
    ...
    try:
        ...
        logger.info(f"  Available images: {len(list(provider.get_all_images()))}")
