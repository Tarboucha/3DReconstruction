...
def example_01_simple_feature_detection():
    ...
    try:
        sift_detector = fds.create_traditional_detector('SIFT', max_features=1000)
        orb_detector = fds.create_traditional_detector('ORB', max_features=1000)
...
def example_04_pipeline_with_presets():
    ...
    for preset in presets_to_test:
        try:
            ...
            pipeline = fds.create_pipeline(preset)
...
def example_05_custom_configuration():
    ...
    custom_config = fds.create_custom_config(
        methods=['SIFT', 'ORB'],
        max_features=1500,
    ...
    validation = fds.validate_config(custom_config)
    ...
    # Print configuration
    fds.print_config(custom_config, "Custom Configuration")
...
def example_07_deep_learning_methods():
    ...
    try:
        ...
        if 'SuperPoint' in info['available_deep_learning']:
            ...
            try:
                superpoint = fds.create_deep_learning_detector('SuperPoint', max_features=1000)
...
def example_09_benchmarking_and_performance():
    ...
    for method in methods_to_benchmark:
        try:
            ...
            detector = fds.create_traditional_detector(method, max_features=1000)
            
            for i, img in enumerate(test_images):
                size_str = f"{img.shape[1]}x{img.shape[0]}"
                
                # Benchmark this detector on this image
                benchmark_result = fds.benchmark_detector_performance(detector, [img], num_runs=3)
...
def example_10_advanced_filtering_and_analysis():
    ...
    try:
        ...
        if len(matches) >= 8:
            ...
            fund_matches, F = fds.verify_matches_with_fundamental_matrix(
            ...
            if result['match_data'].homography is not None:
                errors = fds.calculate_reprojection_error(
        ...
        # Remove duplicates
        unique_matches = fds.remove_duplicate_matches(matches, distance_threshold=2.0)
